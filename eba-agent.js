...
function reconstructSentence(tree, hotReplacements = {}) {
  const tokens = _(flattenTree(tree)).map('token').sortBy('id').value()
  const start = _.head(tokens).start
  
  const reconstruct = (data) => {
    const [pair, token] = data
    const n = token.start - pair[0]
    const form = hotReplacements[token.form] || token.form
    const text = pair[1] + _.repeat(' ', n) + form
    const ext = {
      'start': pair[1].length + n,
      'length': form.length,
      'form': form
    }
    const tok = _.extend(token, ext)
    return [[pair[0] + token.length + n, text], tok]
  }
  
  const defaultAcc = [[start, ""], []]
  const sentenceConstruct = _.reduce(tokens, (acc, tok) => {
    const data = reconstruct([acc[0], tok])
    const [pair, token] = data
    return [pair, _.concat(acc[1], token)]
  }, defaultAcc)
  
  return sentenceConstruct[0][1]
}

module.exports = {Params, Result, LazyLike, LazyValue, GenericLazyExecutor, NLToken, mapTree, reduceTree, flattenTree, makeAnnotation, insertAnnotation, hasAnnotation, reconstructSentence}
